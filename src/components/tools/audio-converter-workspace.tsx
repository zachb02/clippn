...
export function AudioConverterWorkspace() {
  ...
  async function handleConvert() {
    ...
    try {
      ...
      const response = await fetch("/api/media/convert-audio", { method: "POST", body: formData });
      if (!response.ok) {
        const body = await response.json().catch(() => ({}));
        toast.error(body.error ?? "Could not convert this file.");
        return;
      }
      ...
      a.href = url;
      a.download = `clippn-converted.${format}`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      setTimeout(() => URL.revokeObjectURL(url), 1000);
      toast.success(`Converted to ${format.toUpperCase()} and downloaded.`);
    } catch {
      toast.error("Network error while converting.");
    } finally {
      setProcessing(false);
    }
  }

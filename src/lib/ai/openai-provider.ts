 ...
 *
 * Phase 3/4/5 scope, honestly: text generation, model discovery, image
 * generation, image editing, transcription, and speech synthesis are
 * implemented and structurally correct against the SDK's actual types.
 * synthesizeSpeech returns a data: URL (the raw generated bytes,
 * base64-encoded) rather than writing a file itself -- same pattern as
 * generateImage/editImage -- so the adapter stays a pure function with no
 * filesystem side effects; the caller decides where (or whether) to
 * persist it.
 *
...
export const openaiProvider: AIProvider = {
  ...
  async synthesizeSpeech(input: SpeechSynthesisInput, credential: DecryptedCredential): Promise<SpeechResult> {
    const openai = client(credential);
    ...
    return { audioUrl: `data:audio/mpeg;base64,${buffer.toString("base64")}`, modelId: input.modelId };
  },
